...
async def scrape_bigcommerce(config: VendorConfig, client: httpx.AsyncClient) -> list[BikeRecord]:
    ...
    for path in paths:
        path_bikes, pages = await _scrape_bigcommerce_path(config, client, path, seen_urls, now)
