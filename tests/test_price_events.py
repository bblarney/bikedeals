...
def test_unchanged_price_writes_no_event():
    rows = price_event_rows([make_record(price_sale=1500.0)], prior={"bike-1": 1500.0})
    assert rows == []


def test_changed_price_writes_an_event():
    rows = price_event_rows([make_record(price_sale=1300.0)], prior={"bike-1": 1500.0})
    assert len(rows) == 1
...
def test_sub_cent_wobble_is_not_a_change():
    ...
    rows = price_event_rows([make_record(price_sale=1500.004)], prior={"bike-1": 1500.0})
    assert rows == []
...
def test_mixed_batch_only_emits_new_and_changed():
    ...
    prior = {"same": 999.0, "dropped": 1000.0}
